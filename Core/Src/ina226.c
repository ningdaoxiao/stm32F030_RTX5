...
//    dataToSend[2] = (uint8_t)(config_value & 0xFF); // 低字节 0x2
    ...
    HAL_I2C_Mem_Write(&hi2c1, INA226_ADR << 1, INA226_CFG_REG, I2C_MEMADD_SIZE_8BIT, (uint8_t *)&config_value, sizeof(config_value), HAL_MAX_DELAY);
...
{
    uint8_t data[2];
    HAL_I2C_Master_Transmit(hi2c, INA226_ADR << 1, &reg, 1, HAL_MAX_DELAY); // 发送寄存器地址
    HAL_I2C_Master_Receive(hi2c, INA226_ADR << 1, data, 2, HAL_MAX_DELAY); // 接收2字节数据
...
float INA226_ReadCurrent(I2C_HandleTypeDef *hi2c)
{
    ...
    {
        // 如果最高位为 0
        return raw_current * 10; // 根据校准值转换为电流（单位：uA）
        // return raw_current * 5; // 根据校准值转换为电流（单位：uA）
    }
}
